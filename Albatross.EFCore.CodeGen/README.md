...
* Crate the entity mapping class for the entity
  ...
  ```
* The code generator will pick up on the `AddressEntityMap` class and generate the following code.  The
  `CodeGenExtensions` class is placed in the `<RootNamespace>.AutoGenerated` namespace of the project being
  compiled.
  ```csharp
  ...
  ```
* To consume the generated code, import that namespace and use it directly in your custom `DbContext` class
  ```csharp
