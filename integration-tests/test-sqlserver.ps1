...
Describe "Crm.Admin SqlServer Commands" {
    Context "create-company" {
        It "should return name conflict error when creating a company with duplicate name" {
            ...
            $result = & $script:AdminPath sqlserver create-company --name $companyName --description "Test company" 2>&1
            ...
            $result = & $script:AdminPath sqlserver create-company --name $companyName --description "Duplicate test company" 2>&1
